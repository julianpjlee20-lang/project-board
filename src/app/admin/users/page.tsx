...
function UsersContent() {
  ...
  const [pendingResetPassword, setPendingResetPassword] = useState<User | null>(null)
  ...
  async function handleResetPasswordConfirmed() {
    ...
    try {
      const res = await fetch(`/api/admin/users/${pendingResetPassword.id}/reset-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      setToast({ type: 'success', message: `已重設「${pendingResetPassword.name || pendingResetPassword.email}」的密碼` })
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {(user.login_method === 'credentials' || user.login_method === 'both') && (
                          <button
                            ...
                            className="px-2.5 py-1.5 text-xs rounded-md border border-amber-200 text-amber-600 hover:bg-amber-50 transition-colors"
                          >
                            重設密碼
                          </button>
                        )}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              為「{pendingResetPassword?.name || pendingResetPassword?.email}」設定新密碼
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setPendingResetPassword(null)}
